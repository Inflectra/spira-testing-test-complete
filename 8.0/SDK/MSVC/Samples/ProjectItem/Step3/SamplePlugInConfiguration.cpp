// SamplePlugInConfiguration.cpp : Implementation of CSamplePlugInConfiguration
...
CSamplePlugInConfiguration::CSamplePlugInConfiguration()
{
}


void CSamplePlugInConfiguration::Initialize(CComBSTR name,
                                            CComBSTR location,
...
HRESULT STDMETHODCALLTYPE CSamplePlugInConfiguration::get_ParameterCount(long * Value)
{
	_ASSERT( Value != NULL );
	*Value = 2;
	return S_OK;
...
HRESULT STDMETHODCALLTYPE CSamplePlugInConfiguration::get_ParameterCaption(long Index, BSTR * Value)
{
	_ASSERT( Value != NULL );
	switch (Index)
	{
...
HRESULT STDMETHODCALLTYPE CSamplePlugInConfiguration::get_ParameterValue(long Index, VARIANT * Value)
{
	_ASSERT( Value != NULL );
	switch (Index)
	{
...
HRESULT STDMETHODCALLTYPE CSamplePlugInConfiguration::put_ParameterValue(long Index, VARIANT Value)
{
	switch (Index)
	{
...
HRESULT STDMETHODCALLTYPE CSamplePlugInConfiguration::IsValid(long * InvalidParamIndex, VARIANT_BOOL * Value)
{
	_ASSERT( Value != NULL );
